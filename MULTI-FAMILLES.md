...
La racine du site affiche une landing page statique aux visiteurs déconnectés. Les routes `#inscription` et `#connexion` ouvrent l'authentification existante ; une session déjà ouverte rejoint automatiquement `#dashboard`. La comparaison tarifaire présente l'offre Premium comme prochainement disponible : aucun paiement ni quota commercial n'est activé tant que le parcours d'abonnement, les contrôles serveur et les CGV correspondantes ne sont pas livrés.
