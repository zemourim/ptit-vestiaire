...
function ContactLink() {
  return <a className="font-black text-cyan-800 underline" href="#contact">notre formulaire de contact</a>;
}
...
function Faq() {
  const items: Array<[string, ReactNode]> = [
    ...
    ['Comment demander la suppression de mes données ?', <>Supprimez d’abord les vêtements depuis la garde-robe. Pour une suppression complète du compte et des données restantes, utilisez <ContactLink /> en indiquant l’adresse associée au compte.</>]
  ];
...
function Cgu() {
  return <Page eyebrow="Cadre du service" title="Conditions générales d’utilisation">
    ...
    <Section title="2. Accès au service"><p>L’utilisateur doit être majeur, fournir une adresse email valide et préserver la confidentialité de ses identifiants. Il est responsable des actions réalisées depuis son compte et doit signaler sans délai tout accès suspect.</p><p>Une formule gratuite et une formule payante sont proposées. Les conditions commerciales de l’abonnement figurent dans les <a className="font-black text-cyan-800 underline" href="#cgv">conditions générales de vente</a>.</p></Section>
    ...
    <Section title="8. Suspension et suppression"><p>Un compte ou un accès peut être suspendu en cas d’atteinte à la sécurité, d’usage illicite ou de violation grave des présentes conditions. L’utilisateur peut demander la suppression de son compte et de ses données avec <ContactLink />.</p></Section>
...
function Cgv() {
  return <Page eyebrow="Abonnement" title="Conditions générales de vente">
    <p className="text-sm font-bold text-slate-500">Version du {lastUpdate}</p>
    <Section title="1. Vendeur et objet"><p>Les présentes conditions régissent les abonnements PtitVestiaire vendus par {publisherName}, {publisherLegalForm} au capital de {publisherCapital}, immatriculée sous le numéro {publisherRegistration}, joignable à <ContactLink />.</p></Section>
    <Section title="2. Formules et prix"><p>La formule gratuite est accessible sans paiement dans les limites affichées sur le site. La formule payante est proposée à 2,99 € par mois ou 24,99 € par an, prix toutes taxes comprises sauf indication contraire avant validation de la commande. Elle débloque les fonctionnalités indiquées dans le comparatif en vigueur au moment de la souscription.</p></Section>
    <Section title="3. Commande et paiement"><p>Le propriétaire de la famille choisit une périodicité puis paie sur la page sécurisée de Stripe. La commande devient définitive après acceptation des présentes conditions et confirmation du paiement. PtitVestiaire ne reçoit ni ne conserve le numéro complet de la carte bancaire.</p></Section>
    <Section title="4. Durée et renouvellement"><p>L’abonnement est conclu pour un mois ou un an selon la formule choisie et se renouvelle automatiquement pour la même durée. Le prix est prélevé à chaque échéance. Des rappels sont envoyés avant l’échéance selon la périodicité indiquée dans l’application.</p></Section>
    <Section title="5. Gestion et résiliation"><p>Le propriétaire peut modifier son moyen de paiement ou annuler le renouvellement depuis le portail Stripe accessible dans Réglages. L’annulation prend effet à la fin de la période déjà payée. Sauf obligation légale contraire, la période commencée n’est pas remboursée au prorata.</p></Section>
    <Section title="6. Échec de paiement"><p>En cas d’échec, un délai de grâce de trois jours pour un abonnement mensuel ou sept jours pour un abonnement annuel est appliqué. Sans régularisation, la famille repasse automatiquement à la formule gratuite. Les données dépassant ses limites sont conservées mais archivées jusqu’à une éventuelle réactivation.</p></Section>
    <Section title="7. Droit de rétractation"><p>Le consommateur dispose en principe de quatorze jours pour exercer son droit de rétractation avec <ContactLink />. Lorsqu’il demande l’exécution immédiate du service avant la fin de ce délai, les conséquences prévues par la loi, notamment le paiement proportionnel du service déjà fourni le cas échéant, s’appliquent. Les droits légaux du consommateur restent intégralement préservés.</p></Section>
    <Section title="8. Réclamations et droit applicable"><p>Toute réclamation peut être adressée à <ContactLink />. Les présentes conditions sont soumises au droit français, sans priver le consommateur des protections impératives applicables. En cas de litige, une solution amiable sera recherchée avant toute procédure ; les informations relatives au médiateur de la consommation seront ajoutées avant l’ouverture commerciale en mode live.</p></Section>
  </Page>;
}
...
function Confidentialite() {
  return <Page eyebrow="Vos données" title="Politique de confidentialité">
    ...
    <Section title="Données traitées"><p>PtitVestiaire traite les données de compte (identifiant Firebase, email, fournisseur de connexion et état de validation), le nom de la famille et ses membres, les prénoms des enfants, les photos ajoutées, les descriptions de vêtements, les mouvements de sortie ou de retour, les réglages et les données techniques de sécurité. Le formulaire de contact traite le nom, l’adresse de réponse, le sujet et le message saisis ; une empreinte technique quotidienne de l’adresse réseau limite les envois abusifs. Pour la formule payante, PtitVestiaire traite également les identifiants Stripe, la périodicité, le statut et les échéances de l’abonnement ; les données complètes de carte restent chez Stripe.</p></Section>
...
function MentionsLegales() {
  return <Page eyebrow="Informations légales" title="Mentions légales">
    <p className="text-sm font-bold text-slate-500">Version du {lastUpdate}</p>
    <Section title="Éditeur"><p>PtitVestiaire est édité par {publisherName}, {publisherLegalForm} au capital social de {publisherCapital}, immatriculée sous le numéro {publisherRegistration}.</p><p>Siège social : {publisherAddress}<br />Contact : <ContactLink /><br />Téléphone : <a className="font-black text-cyan-800 underline" href="tel:+33621344567">{publisherPhone}</a></p><p>Direction de la publication : {publisherName}.</p></Section>
